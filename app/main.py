...
from app.services.whatsapp import send_whatsapp_message, check_venom_service_health
...
app = FastAPI(
    ...
    description="A WhatsApp-based booking system for salons using FastAPI, Firebase and Venom Bot",
    version="1.0.0"
)
...
async def startup_event():
    ...
    # Check Venom service
    logger.info("Checking Venom Bot service...")
    if check_venom_service_health():
        logger.info("✅ Venom Bot service is ready!")
    else:
        logger.warning("⚠️ Venom Bot service is not ready. Please make sure to start the Venom Bot service first.")
...
async def root():
    ...
    venom_status = "ready" if check_venom_service_health() else "not ready"
    return {
        "status": "healthy", 
        "message": "Smart WhatsApp Booking Bot is running",
        "venom_service": venom_status
    }
...
@app.post("/webhook/venom")
async def venom_webhook(request: Request):
    """Webhook endpoint for Venom Bot messages"""
    try:
        logger.info("Received Venom webhook request")
        
        # Get JSON data from Venom Bot service
        data = await request.json()
        logger.info(f"Venom data: {data}")
        
        message = data.get("body", "").lower().strip()
        phone = data.get("from", "").replace("@c.us", "")
        
        if not message or not phone:
            logger.error("Missing message or phone in Venom webhook")
