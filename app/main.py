...
async def generate_real_qr(salon_id: str):
    """Proxy live QR code from WhatsApp service - always fresh and valid"""
    try:
        # Salon port mapping for unified service
        salon_ports = {
            "salon_a": "3005",
            "salon_b": "3006", 
            "salon_c": "3007"
        ...
        if salon_id not in salon_ports:
        ...
        # Log QR access
        log_qr_access(salon_id, "live_proxy")
        
        try:
            # Connect directly to live WhatsApp service for fresh QR
            whatsapp_url = f"http://localhost:{salon_port}"
            response = requests.get(f"{whatsapp_url}/qr-image", timeout=10)
            
            if response.status_code == 200:
                logger.info(f"✅ Retrieved LIVE QR from WhatsApp service for {salon_name} ({salon_id})")
                return Response(
                    content=response.content,
                    media_type="image/png",
                    headers={
                        "Cache-Control": "no-cache, no-store, must-revalidate",  # Never cache QR codes
                        "Pragma": "no-cache",
                        "Expires": "0",
                        "Content-Disposition": f"inline; filename=live_qr_{salon_id}.png"
                    }
                )
            else:
            ...
            logger.warning(f"⚠️ Could not get live QR from WhatsApp service: {service_error}")
            
            # Fallback: Create a "service not ready" message
            ...
            error_text = f"{salon_name}\n\nWhatsApp Service Not Running\n\nTo get fresh QR codes:\n1. Start unified service locally\n2. Visit: localhost:{salon_port}/qr\n3. Scan the live QR code\n\nQR codes expire quickly!"
        ...
        logger.error(f"❌ Error getting live QR for {salon_id}: {e}")
        ...
        error_text = f"QR Code Service Error\n\nSalon: {salon_id}\n\nWhatsApp Web QR codes:\n- Expire quickly (20 seconds)\n- Change with each restart\n- Must be live/fresh to work\n\nStart the service to get live QR codes"
