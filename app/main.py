...
app = FastAPI(
    ...
    description="A simple WhatsApp-based booking system for a single salon",
    version="3.0.0"
)
...
async def firebase_status():
    ...
    try:
        ...
        if firebase_connected:
            status_data["message"] = f"✅ Firebase connected - using cloud database for {settings.SALON_NAME}"
        else:
...
async def qr_code_page():
    ...
    <html>
    <head>
        <title>{settings.SALON_NAME} - WhatsApp QR Code</title>
    ...
    <body>
        <div class="container">
            <h1>🔄 Redirecting to {settings.SALON_NAME} WhatsApp QR Code...</h1>
